...
def register(im1, im2, params, Out_p, exact_params=False, verbose=1):
    #global TransofrmationParameter
    ...
    * verbose (int):
    ...
    path_trafo_mhd = os.path.join(tempdir, 'im2.mhd')
    ...
    if True and params["Transform"] == "SimilarityTransform" :
        with open(path_trafo_params) as Total_Info:
            for Items in Total_Info:
                if "(TransformParameters "  in Items:
                    Info = Items.replace("(", "")
                    Info = Info.replace(")", "")
                    Info = Info.split(" ")
        New_Path = os.path.dirname(Out_p)
        New_Path_Tp = os.path.join(New_Path, "NRTransformParameters.0.txt")
        shutil.move(path_trafo_params, New_Path_Tp)
        # os.renames(path_trafo_params, New_Path_Tp)
        #Info = pd.read_csv(path_trafo_params, delimiter=" ") 
        _clear_temp_dir()
        return a, fields, Info
    if True and params["Transform"] == "BSplineTransform" :
        New_Path = os.path.dirname(Out_p)
        New_Path_Tp = os.path.join(New_Path, "NRRTransformParameters.0.txt")
        # New_Path_mhd = os.path.join(New_Path, "im2.mhd")
        shutil.move(path_trafo_params, New_Path_Tp)
        # os.renames(path_trafo_params, New_Path_Tp)
        # os.renames(path_trafo_mhd, New_Path_mhd)
        print(path_trafo_params)
        return a, fields
    # Clean and return
    _clear_temp_dir()
    return a, fields
...
def get_default_params(type='BSPLINE'):
    ...
    * MaximumNumberOfIterations (int):
    ...
    if type in ['B', 'BSPLINE', 'B-SPLINE']:
        ...
        #p.FinalGridSpacingInPhysicalUnits = 32
    
    if type in ['RIGID', 'EULER', 'AFFINE','SIMILARITY']:
        
        # Affine or Euler transform
        if type in ['RIGID', 'EULER']:
            p.Transform = 'EulerTransform'
            
        elif type  in ['SIMILARITY']:
            p.Transform = 'SimilarityTransform'
            p.Scales = 1000,1000,1000
            #p.CenterOfRotation = 128,128,90
    ...
    if type in ['RIGID', 'EULER', 'AFFINE','SIMILARITY']:
        p.MaximumStepLength = 5.0 #Default uses voxel spaceing
    if type in ['B', 'BSPLINE', 'B-SPLINE']:
